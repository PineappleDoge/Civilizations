local FFlagUserFixSlowTouchCameraInputOverwrite
do
	local success, result = pcall(function()
		return UserSettings():IsUserFeatureEnabled("UserFixSlowTouchCameraInputOverwrite")
	end)
	FFlagUserFixSlowTouchCameraInputOverwrite = success and result
end
		...
		local touchBegan, touchChanged, touchEnded, resetTouchState do
			...
			function touchChanged(input, sunk)
				...
				if #unsunkTouches == 1 then
					if touches[input] == false then
						local delta = input.Delta
						if FFlagUserFixSlowTouchCameraInputOverwrite then
