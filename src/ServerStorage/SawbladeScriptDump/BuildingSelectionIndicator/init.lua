...
function BuildingSelectionIndicator.new()
	...
	self.Maid:GiveTask(self.BaseMaid)

	for i = 1, 4 do
		local attachmentBase: Attachment = Instance.new("Attachment")
		self.Maid:GiveTask(attachmentBase)
...
function BuildingSelectionIndicator:_SetBase(buildingBasePart: BasePart)
	...
	updateAttachments()
	self.BaseMaid:GiveTask(buildingBasePart.Changed:Connect(updateAttachments))
