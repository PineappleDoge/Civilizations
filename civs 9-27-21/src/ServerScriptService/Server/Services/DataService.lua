...
local Janitor = require(REPLICATED_STORAGE.Shared.Knit.Util.Janitor)
local RemoteProperty = require(REPLICATED_STORAGE.Shared.Knit.Util.Remote.RemoteProperty)
...
function OnPlayerJoin(Player: Player)
	...
	task.defer(function()
		BuildingService:SpawnPlayerBuildings(Player, DataService.UnserializedData[UserID].BaseData)
	end)
end
...
function SetupPlayerCanvasInWorkspace(Player, Profile, Skin)
...
local lookup = {
	-- [652000534] = function() GameStore:WipeProfileAsync("Player_652000534") end,
}
...
function DataService:OnPlayerLeave(Player: Player)
	...
	for name, child in pairs(DataService.UnserializedData[UserID]) do
		if RemoteProperty.Is(child) == true then
