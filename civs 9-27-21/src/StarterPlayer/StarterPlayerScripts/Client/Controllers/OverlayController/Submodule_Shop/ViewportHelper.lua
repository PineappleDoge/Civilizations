...
function ViewportModelClass.new(vpf: ViewportFrame, camera: Camera)
...
local header = screen.Container.Frame.Header
local body = screen.Container.Frame.Body

local Models = game.ReplicatedStorage.Models
local ViewportModel = require(game.ReplicatedStorage.ViewportModel)

require(script.Radio)(header, body)

-- Grid

local scroll = body.Grid.ScrollingFrame
local content = scroll.Content
local template = content.Template

for index, model in pairs(Models:GetChildren()) do
...
template:Destroy()
scroll.CanvasSize = UDim2.new(0, 0, 0, content.UIGridLayout.AbsoluteContentSize.Y)

-- Min Fit

local scroll = body.MinFit.ScrollingFrame
local content = scroll.Content
local template = content.Template

local vpf = body.MinFit.ViewportFrame
...
local vpfModel = ViewportModel.new(vpf, camera)

local theta = 0
local orientation = CFrame.new()
local prevModel = Instance.new("Model")

game:GetService("RunService").RenderStepped:Connect(function(dt)
	theta = theta + math.rad(20 * dt)
	orientation = CFrame.fromEulerAnglesYXZ(math.rad(-20), theta, 0)
	camera.CFrame = vpfModel:GetMinimumFitCFrame(orientation)
end)

for _, model in pairs(Models:GetChildren()) do
	local list = template:Clone()
	list.Text = model.Name
	list.Parent = content
	...
	list.Activated:Connect(function()
		...
		vpfModel:SetModel(copy)
	end)
end

template:Destroy()
scroll.CanvasSize = UDim2.new(0, 0, 0, content.UIListLayout.AbsoluteContentSize.Y)
]]